...
class Auth {
            ...
            .then(body => {
                if (!body.active) {
                    return Promise.reject(
                        new Error('Bearer token is not active'));
                } else if (body.token_type && body.token_type
                    !== 'access_token') {
                    return Promise.reject(
    ...
    checkJWT(acp, entity) {
        ...
        if (introspectionToken.aud !== undefined
    ...
    checkJWTAuth(token, entity) {
        ...
        return jwt.verify(token.toString(), this.jwtTokenMap[entity],
    ...
    wardenSubject(acp) {
        const url = KETO_URL + '/warden/subjects/authorize';
        logger.debug(url);
        return this.authorisedRequest('POST', url, acp)
            .then(body => {
                if (!body.allowed) {
                    return Promise.reject('Request was not allowed');
                }
                return Promise.resolve(body);
            })
            .catch((error) => {
    ...
    wardenToken(acp) {
        logger.debug('oauth.wardentoken');
        const url = KETO_URL + '/warden/oauth2/access-tokens/authorize';
        return this.authorisedRequest('POST', url, acp)
            .then(body => {
                if (!body.allowed) {
    ...
    refresh() {
        ...
        logger.debug(process.env.HTTPS);
        logger.debug(process.env.OAUTH2_TOKEN_URL);
        const params = {};
        if (process.env.HTTPS !== undefined) {
            params.headers = {
        ...
        if (this.token) {
            if (this.token.expired()) {
                return this.token.refresh(params)
                    .then((t) => {
                        this.token = t;
