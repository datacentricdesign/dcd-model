...
export class Policies {
    model: DCDModel;
    ketoUrl: url.UrlWithStringQuery;
    ketoHeaders: { 'Content-Type': string; Accept: string };
    ...
    readPolicy(subjectId, resourceId, roleName) {
        return this.model.dao.readRoleId(subjectId, resourceId, roleName).then(sqlResult => {
            ...
            return Promise.reject(
                new DCDError(4041, 'Role not found for ' + subjectId + ', ' + resourceId + ' and ' + roleName),
            );
        });
    }
     ...
     */
    updateKetoPolicy(policy): Promise<void> {
