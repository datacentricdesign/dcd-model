...
export class InteractionService extends Service {
     ...
     * returns interaction
     **/
    create(interaction): Promise<Interaction> {
        return this.readByIdOrEntityId(interaction)
            ...
            .catch(error => {
                ...
                if (error.code === 404) {
                    this.logger.debug('create: Interaction does not exist, sending it to Kafka');
    ...
    readByIdOrEntityId(interaction): Promise<Interaction> {
        if (interaction.id === undefined) {
     ...
     **/
    list(actorId, entityDestId): Promise<Interaction[]> {
     ...
     * returns {Interaction}
     **/
    read(id: string): Promise<Interaction> {
     ...
     * returns {Interaction}
     **/
    readByEntityId(entityId1, entityId2): Promise<Interaction> {
        let interaction: Interaction;
        return this.model.dao
     ...
     * @return {Promise}
     */
    del(interactionId): Promise<DeletionReport> {
     ...
     * @param {Interaction} interaction
     */
    toKafka(interaction): Promise<void> {
        return this.model.kafka.pushData('interactions', [interaction], interaction.id);
    }
}
